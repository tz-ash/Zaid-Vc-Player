...
async def generate_cover(requested_by, title, views, duration, thumbnail):
    ...
    image2 = Image.open("etc/1_5039778796232769879.png")
...
def updated_stats(chat, queue, vol=100):
    if chat.id in callsmusic.pytgcalls.active_calls:
        ...
        if len(que) > 0:
            ...
            stats += "Qᴜᴇᴜᴇ ɴᴜᴍʙᴇʀ: `{}`\n".format(len(que))
            stats += "ꜱᴏɴɢ ɴᴀᴍᴇ: **{}**\n".format(queue[0][0])
            stats += "ᴋɪᴅ ᴜꜱᴇʀ ʙʏ: {}".format(queue[0][1].mention)
    else:
        stats = None
    return stats
...
async def play(_, message: Message):
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    # print(e)
                    await lel.edit(
                        f"<b>⛑ Flood Wait Error ⛑\n{user.first_name} ᴋɪᴅ helper is not in Ur chat plz add Manually."
                        f"\n\Kid Assistant @{ASSISTANT_NAME} try to add manually</b>",
                    )
    try:
    ...
    if audio:
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    ...
                    InlineKeyboardButton("📣 ᴄʜᴀɴɴᴇʟ", url=f"https://t.me/kidbots")
                ],
            ]
        )
        file_name = get_file_name(audio)
        title = file_name
        thumb_name = "https://telegra.ph/file/77983ffd643c960975298.png"
        ...
        try:
            ...
            while j < 6:
                ...
                toxxt += f" └ ⚡ __ᴘᴏᴡᴇʀᴇᴅ ʙʏ {BOT_NAME} ᴋɪᴅ__\n\n"
                j += 1            
            keyboard = InlineKeyboardMarkup(
                [
                    [
                    ...
                    [InlineKeyboardButton(text="🗑ᴋ ᴄʟᴏꜱᴇ", callback_data="cls")],
                ]
            )
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        await message.reply_photo(
            photo="final.png",
            caption=f"💡 **ᴛʀᴀᴄᴋ ᴀᴅᴅᴇᴅ ɪɴ Qᴜᴇᴜᴇ**\n\n🏷 **ɴᴀᴍᴇ:** [{title[:45]}]({url})\n⏱ **ᴅᴜʀᴀᴛɪᴏɴ:** `{duration}`\n🎧 **ᴋɪᴅ ᴜꜱᴇʀ ʙʏ:** {message.from_user.mention}\n" \
        ...
        await message.reply_photo(
            ...
            caption=f"🏷 **ɴᴀᴍᴇ:** [{title[:45]}]({url})\n⏱ **ᴅᴜʀᴀᴛɪᴏɴ:** `{duration}`\n😍 **ꜱᴛᴀᴛᴜꜱ:** `ᴘʟᴀʏɪɴɢ`\n" \
                   +f"🎧 **ᴋɪᴅ ᴜꜱᴇʀ ʙʏ:** {message.from_user.mention}",
            reply_markup=keyboard
        )
        os.remove("final.png")
...
async def ytplay(_, message: Message):
    ...
    for i in message.command[1:]:
        query += " " + str(i)
    print(query)
    await lel.edit("🎵 **ᴄᴏɴɴᴇᴄᴛɪɴɢ ᴛᴏ ᴋɪᴅ ꜱᴇʀᴠᴇʀ...**")
    ydl_opts = {"format": "bestaudio[ext=m4a]"}
    try:
        results = YoutubeSearch(query, max_results=1).to_dict()
        url = f"https://youtube.com{results[0]['url_suffix']}"
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                ...
                InlineKeyboardButton("📣 ᴄʜᴀɴɴᴇʟ", url=f"https://t.me/kidbots"),
                InlineKeyboardButton("✨ ɢʀᴏᴜᴘ", url=f"https://t.me/tzkid")
            ],
        ]
    )
    requested_by = message.from_user.first_name
